    ...
    <div class="min-h-[calc(100%-56px)] w-full p-4 bg-gray-100">
        ...
        <div class="w-full">
            <div class="w-full mt-5 flex justify-center">
                <form action="{{ route('users.update', ['key' => $key]) }}" method="POST" class="w-full sm:!w-[500px]">
                    @csrf
                    <div class="mb-4">
                        <label for="" class="text-gray-700 font-bold">ID Number <span class="text-red-500">*</span></label>
                        <input type="text" name="id" value="{{ $user->id_number }}" class="{{ old('id') == '' && session('error') ? 'border-red-500' : '' }} w-full lg:w-1/2 border-gray-300 rounded-lg shadow-inner" autocomplete="off">
                    </div>
                    <div class="mb-8">
