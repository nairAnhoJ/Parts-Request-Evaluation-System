@extends('layouts.app')
@section('title','EVALUATION FORM - EDIT')
@section('content')
    ...
    <div class="min-h-[calc(100%-56px)] w-full p-4 bg-gray-100">
        ...
        <div class="w-full">
            <div class="w-full mt-5 flex justify-center">
                <form action="{{ route('form.update') }}" method="POST" class="w-full sm:!w-5/6">
                    @csrf
                    ...
                    <div class="grid grid-cols-3 gap-x-5">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="number" name="number" value="{{ $form->number }}" class="w-full lg:w-1/2 border-gray-300 rounded-lg shadow-inner" autocomplete="off" required>
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="text" id="control_number" name="control_number" value="{{ $form->control_number }}" class="w-full lg:w-1/2 border-gray-300 rounded-lg shadow-inner" autocomplete="off" required>
                        </div>
                        <div class="mb-3">
                            <label for="sq_number" class="block text-sm font-semibold text-gray-600">SQ Number <span class="text-red-500">*</span></label>
                            <input type="text" id="sq_number" name="sq_number" value="{{ $form->sq_number }}" class="w-full lg:w-1/2 border-gray-300 rounded-lg shadow-inner" autocomplete="off" required>
                        </div>
                    </div>
                    <div class="grid grid-cols-3 gap-x-5">
                        <div id="nameSelect" class="flex flex-col relative optionDiv mb-3 col-span-2">
                            ...
                            <input type="text" id="name" name="name" value="{{ $form->customer->name }}" class="inputOption block w-full p-2 text-gray-600 border border-gray-300 rounded-lg bg-gray-50 sm:text-sm" autocomplete="off" required>
                        ...
                        <div class="mb-3 flex flex-col">
                            ...
                            <input type="text" id="area" name="area" value="{{ $form->customer->area }}" class=" w-full lg:w-1/2 border-gray-300 rounded-lg shadow-inner" autocomplete="off" required>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label for="address" class="text-gray-700 font-bold text-sm">Address <span class="text-red-500">*</span></label>
                        <input type="text" id="address" name="address" value="{{ $form->customer->address }}" class="w-full lg:w-1/2 border-gray-300 rounded-lg shadow-inner" autocomplete="off" required>
                    </div>
                    <div class="grid grid-cols-3 gap-x-5">
                        <div class="mb-3">
                            ...
                            <select id="brand" name="brand" class="bg-gray-50 border border-gray-300 text-gray-600 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                @foreach ($brands as $brand)
                                    <option {{ ($form->brand == $brand->id)? 'selected' : '' }} value="{{ $brand->id }}">{{ $brand->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div id="modelSelect" class="flex flex-col relative optionDiv mb-3 w-full">
                            ...
                            <input type="text" id="model" name="model" value="{{ $form->model }}" class="inputOption block w-full p-2 text-gray-600 border border-gray-300 rounded-lg bg-gray-50 sm:text-sm" autocomplete="off" required>
                            <div class="listOption hidden absolute top-[65px] w-full rounded-lg border border-gray-300 overflow-y-auto max-h-[30vh] text-gray-600 bg-white z-[99] shadow-xl">
                                <ul id="modelOptions">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="serial_number" name="serial_number" value="{{ $form->serial_number }}" class="w-full lg:w-1/2 border-gray-300 rounded-lg shadow-inner" autocomplete="off" required>
                        </div>
                    </div>
                    <div class="grid grid-cols-3 gap-x-5">
                        <div class="mb-3">
                            <label for="fsrr_number" class="block text-sm font-semibold text-gray-600">FSRR Number <span class="text-red-500">*</span></label>
                            <input type="text" id="fsrr_number" name="fsrr_number" value="{{ $form->fsrr_number }}" class="w-full lg:w-1/2 border-gray-300 rounded-lg shadow-inner" autocomplete="off" required>
                        </div>
                        <div class="mb-3 w-full">
                            ...
                            <div class="relative w-full">
                                ...
                                <input datepicker type="text" id="date_received" name="date_received" value="{{ $form->date_received }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" placeholder="Select date" autocomplete="off" required>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label for="technician" class="block text-sm font-semibold text-gray-600">Technician <span class="text-red-500">*</span></label>
                            <input type="text" id="technician" name="technician" value="{{ $form->technician }}" class="w-full lg:w-1/2 border-gray-300 rounded-lg shadow-inner" autocomplete="off" required>
                        </div>
                    </div>
                    <div class="grid grid-cols-3 gap-x-5">
                        <div class="mb-3">
                            <label for="hm" class="block text-sm font-semibold text-gray-600">HM</label>
                            <input type="text" id="hm" name="hm" value="{{ $form->hm }}" class="w-full lg:w-1/2 border-gray-300 rounded-lg shadow-inner" autocomplete="off">
                        </div>
                        <div class="mb-3">
                            <label for="disc" class="block text-sm font-semibold text-gray-600">DISC</label>
                            <input type="text" id="disc" name="disc" value="{{ $form->disc }}" class="w-full lg:w-1/2 border-gray-300 rounded-lg shadow-inner" autocomplete="off">
                        </div>
                        <div class="mb-3 flex">
                            <div class="w-1/2">
                                ...
                                <div class="flex items-center h-10 gap-x-5">
                                    <div class="flex items-center">
                                        <input {{ ($form->working_environment == "COLD") ? 'checked' : '' }} id="cold" type="radio" value="COLD" name="working_environment" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2">
                                        <label for="cold" class="ml-2 text-sm font-medium text-gray-900">COLD</label>
                                    </div>
                                    <div class="flex items-center">
                                        <input {{ ($form->working_environment == "DRY") ? 'checked' : '' }} id="dry" type="radio" value="DRY" name="working_environment" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2">
                            ...
                            <div class="w-1/2">
                                ...
                                <div class="flex items-center h-10 gap-x-5">
                                    <div class="flex items-center">
                                        <input {{ ($form->status == "UP") ? 'checked' : '' }} id="up" type="radio" value="UP" name="status" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2">
                                        <label for="up" class="ml-2 text-sm font-medium text-gray-900">UP</label>
                                    </div>
                                    <div class="flex items-center">
                                        <input {{ ($form->status == "DOWN") ? 'checked' : '' }} id="down" type="radio" value="DOWN" name="status" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2">
                                    ...
                                    @foreach ($form->details as $detail)
                                        <div id="part{{ $count }}" class="flex w-full">
                                            ...
                                            <div class="text-center w-[10%] border-b border-r border-gray-400 pb-2.5 px-5">
                                                <input type="text" name="quantity_{{ $count }}" value="{{ $detail->quantity }}" class="w-full border-0 border-b bg-gray-100 focus:border-blue-500 focus:border-b-2 focus:ring-0 text-center pb-1 pt-3" autocomplete="off" {{ ($count == 1)? 'required' : '' }}>
                                            </div>
                                            <div class="text-center w-[10%] border-b border-r border-gray-400 pb-2.5 px-5">
                                                <input type="text" name="price_{{ $count }}" value="{{ $detail->unit_price }}" class="w-full border-0 border-b bg-gray-100 focus:border-blue-500 focus:border-b-2 focus:ring-0 text-center pb-1 pt-3" autocomplete="off" {{ ($count == 1)? 'required' : '' }}>
                                            </div>
                                            <div class="w-[8%] border-b border-gray-400 flex items-center justify-center">
                                                @if ($count != 1)
                                                    <button type="button" data-count="part{{ $count }}" class="deletePart hover:scale-105">
                    ...
                    <div class="mb-8">
                        ...
                        <input type="text" id="remarks" name="remarks" value="{{ $form->remarks }}" class="w-full lg:w-1/2 border-gray-300 rounded-lg shadow-inner" autocomplete="off">
                    </div>
